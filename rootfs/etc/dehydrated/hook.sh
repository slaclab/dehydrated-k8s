...
deploy_cert() {
    ...
    # Note: `kubectl create secret` can fail if a secret already exists
    # We run it as a dry-run and then just apply the yaml (which won't fail if the secret already exists)
    kubectl create secret tls "${DOMAIN_SECRET}" --key="${KEYFILE}" --cert="${FULLCHAINFILE}" --dry-run=client -o yaml | kubectl apply -f -
}
