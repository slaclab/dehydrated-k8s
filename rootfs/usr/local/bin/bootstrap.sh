...
if [[ ! -f "${CERTDIR}/dhparam.pem" ]]; then
  openssl dhparam -out ${CERTDIR}/dhparam.pem 1024
fi
...
for line in $(<"${DOMAINS_TXT}" tr -d '\r' | awk '{print tolower($0)}' | _sed -e 's/^[[:space:]]*//g' -e 's/[[:space:]]*$//g' -e 's/[[:space:]]+/ /g' -e 's/([^ ])>/\1 >/g' -e 's/> />/g' | (grep -vE '^(#|$)' || true)); do
  ...
  if [[ ! -d ${CERTDIR}/${domain} ]]; then
    ...
    cp ${CERTDIR}/privkey.pem ${CERTDIR}/fullchain.pem ${CERTDIR}/dhparam.pem ${CERTDIR}/${domain}/.
  fi
done
